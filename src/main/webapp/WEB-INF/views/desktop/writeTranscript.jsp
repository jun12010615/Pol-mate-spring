        ...
        .then(function(data) {
            ...
            sel.innerHTML = '<option value="">-- 사건 선택 --</option>' +
                cases.map(function(c) {
                    var sel_attr = c.id === _initCaseId ? ' selected' : '';
                    return '<option value="' + c.id + '"' + sel_attr + '>' + c.id + ' · ' + (c.name||'') + '</option>';
                }).join('');
        }).catch(function() {
    ...
    fetch(_ctx + '/caseApi', {method: 'POST', body: fd, credentials: 'same-origin'})
        ...
        .then(function(d) {
            ...
            if (d.success || d.transcriptId) {
                ...
                setTimeout(function() {
                    location.href = _ctx + '/desktop/myCase?caseId=' + encodeURIComponent(caseId);
                }, 1200);
            } else {
