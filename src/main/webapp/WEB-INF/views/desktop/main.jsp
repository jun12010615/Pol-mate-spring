...
<main class="pm-page">
    ...
    <div id="pmAlertBanner" style="display:none;">
        <a id="pmAlertLink" href="#" class="pm-alert-banner">
            <span class="pm-alert-pulse"></span>
            <span id="pmAlertText"></span>
        </a>
    </div>
    ...
    <div style="margin-bottom:28px;">
        ...
        <div id="pmGreetingHint" style="display:none;" class="pm-greeting-hint"></div>
    </div>

    <div class="pm-stat-grid">
        <div class="pm-stat-card"><div class="pm-stat-val" id="cntActive">—</div><div class="pm-stat-lbl">진행 중인 사건</div></div>
        <div class="pm-stat-card"><div class="pm-stat-val danger" id="cntContradiction">—</div><div class="pm-stat-lbl">모순 탐지 사건</div></div>
        <div class="pm-stat-card"><div class="pm-stat-val" id="cntTranscript">—</div><div class="pm-stat-lbl">작성한 조서</div></div>
        <div class="pm-stat-card"><div class="pm-stat-val danger" id="cntContradictionDoc">—</div><div class="pm-stat-lbl">조서 모순 탐지</div></div>
    </div>

    <div class="pm-sec-label">주요 수사 도구</div>
    <div class="pm-tool-grid">
        <a href="${pageContext.request.contextPath}/desktop/writeTranscript" class="pm-tool-card">
            ...
            <div class="pm-tool-meta" id="metaTranscript">지금까지 — 건</div>
        </a>
        <a href="${pageContext.request.contextPath}/desktop/voiceTranscript" class="pm-tool-card">
            <div class="pm-tool-icon" id="contradictionIcon">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round">
            ...
            <div class="pm-tool-desc">AI로 조서 간 진술 불일치 자동 발견</div>
            <div class="pm-tool-meta" id="metaContradiction">신규 — 건</div>
        </a>
        <a href="${pageContext.request.contextPath}/desktop/caseRelationMap" class="pm-tool-card">
            <div class="pm-tool-icon">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round">
        ...
        <a href="${pageContext.request.contextPath}/desktop/cctvAnalysis" class="pm-tool-card">
            <div class="pm-tool-icon">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round">
    ...
    <div class="pm-lower-grid">
        <div>
            ...
            <div class="pm-case-table">
                ...
                <div id="pmRecentCases">
                    <div style="padding:24px;text-align:center;color:#9ca3af;font-size:13px;">로딩 중...</div>
                </div>
            </div>
        </div>
...
(function() {
    ...
    // ── 사건 목록 (카운터 + 최근 사건 테이블) ─────────────────
    fetch(ctx + '/caseApi?action=caseList', {credentials: 'same-origin'})
        .then(function(r) { return r.json(); })
        .then(function(list) {
            if (!Array.isArray(list)) return;
            var cntActive = 0, cntContra = 0, alertCaseId = null;
            list.forEach(function(c) {
                if (c.status === '진행중')  cntActive++;
                if (c.status === '모순탐지') { cntContra++; if (!alertCaseId) alertCaseId = c.id; }
            });
            document.getElementById('cntActive').textContent        = cntActive;
            document.getElementById('cntContradiction').textContent = cntContra;
            document.getElementById('metaContradiction').textContent = '신규 ' + cntContra + '건';

            if (alertCaseId) {
                document.getElementById('pmAlertLink').href = ctx + '/desktop/myCase?caseId=' + alertCaseId;
                document.getElementById('pmAlertText').innerHTML = '사건 <strong>' + alertCaseId + '</strong>에서 진술 모순이 탐지되었습니다.';
                document.getElementById('pmAlertBanner').style.display = '';
            }
            if (cntContra > 0) {
                var hint = document.getElementById('pmGreetingHint');
                hint.innerHTML = '신규 모순 탐지 <strong style="color:#dc2626">' + cntContra + '건</strong>이 있습니다.';
                hint.style.display = '';
                var dot = document.createElement('span');
                dot.className = 'pm-urgent-dot';
                document.getElementById('contradictionIcon').insertBefore(dot, document.getElementById('contradictionIcon').firstChild);
            }

            // 최근 사건 테이블 (최대 5개)
            var container = document.getElementById('pmRecentCases');
            var recent = list.slice(0, 5);
            if (recent.length === 0) {
                container.innerHTML = '<div style="padding:24px;text-align:center;color:#9ca3af;font-size:13px;">등록된 사건이 없습니다</div>';
                return;
            }
            var badgeMap = {'진행중':'pm-badge-jinhaeng','완료':'pm-badge-wanryo','모순탐지':'pm-badge-moosun'};
            container.innerHTML = recent.map(function(c) {
                var cls = badgeMap[c.status] || 'pm-badge-geomto';
                var urgent = c.status === '모순탐지' ? ' urgent' : '';
                return '<a href="' + ctx + '/desktop/myCase?caseId=' + c.id + '" class="pm-case-row' + urgent + '">'
                    + '<span class="pm-case-id">' + c.id + '</span>'
                    + '<span class="pm-case-name">' + c.name + '</span>'
                    + '<span><span class="pm-badge ' + cls + '">' + c.status + '</span></span>'
                    + '<span class="pm-case-date">' + (c.date || '') + '</span></a>';
            }).join('');
        })
        .catch(function() {
            document.getElementById('pmRecentCases').innerHTML =
                '<div style="padding:24px;text-align:center;color:#9ca3af;font-size:13px;">조회 실패</div>';
        });

    // ── 조서 통계 ─────────────────────────────────────────────
    fetch(ctx + '/caseApi?action=docStats', {credentials: 'same-origin'})
        .then(function(r) { return r.json(); })
        .then(function(d) {
            document.getElementById('cntTranscript').textContent       = d.total        || 0;
            document.getElementById('cntContradictionDoc').textContent = d.contradiction || 0;
            document.getElementById('metaTranscript').textContent      = '지금까지 ' + (d.total || 0) + '건';
        })
        .catch(function() {});

    // ── 최근 활동 (알림 목록) ─────────────────────────────────
    fetch(ctx + '/notifApi?action=list', {credentials: 'same-origin'})
        .then(function(r) { return r.json(); })
        .then(function(d) {
        ...
        .catch(function() {
            document.getElementById('pmActivityFeed').innerHTML =
                '<div style="padding:16px;text-align:center;color:#9ca3af;font-size:12px;">불러올 수 없습니다</div>';
        });
})();
</script>
</body>
