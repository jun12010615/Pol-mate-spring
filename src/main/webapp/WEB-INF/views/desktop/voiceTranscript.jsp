...
(function loadCaseOptions() {
    fetch(_ctx + '/caseApi?action=caseList', {credentials: 'same-origin'})
        ...
        .then(function(data) {
            ...
            if (!cases.length) {
                sel.innerHTML = '<option value="">등록된 사건이 없습니다</option>';
                return;
            }
            sel.innerHTML = '<option value="">-- 사건을 선택하세요 --</option>' +
                cases.map(function(c) {
                    return '<option value="' + c.id + '" data-name="' + (c.name||'').replace(/"/g,'&quot;') + '" data-suspect="' + (c.suspect||'').replace(/"/g,'&quot;') + '" data-status="' + (c.status||'').replace(/"/g,'&quot;') + '">'
                        + c.id + ' · ' + (c.name||'') + '</option>';
                }).join('');
        })
        .catch(function() {
            document.getElementById('caseSelect').innerHTML = '<option value="">사건 목록 로드 실패</option>';
        });
})();
...
function loadDocs(caseId) {
    ...
    fetch(_ctx + '/caseApi?action=caseDetail&caseId=' + encodeURIComponent(caseId), {credentials:'same-origin'})
        ...
        .then(function(d) {
            var docs = d.docs || [];
            if (!docs.length) {
                list.innerHTML = '<div class="doc-empty">이 사건에 등록된 조서가 없습니다.<br><a href="' + _ctx + '/desktop/writeTranscript?caseId=' + encodeURIComponent(caseId) + '" style="color:#4a7cdc;text-decoration:none;">조서 작성하러 가기 →</a></div>';
                return;
            }
